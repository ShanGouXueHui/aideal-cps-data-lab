#!/usr/bin/env bash
# Resume HZ15 jump-pages collector from page 4 to 67 with slower page-jump cadence.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ15 resume jump-pages 4..67 slow ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz15_resume_4_67_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ15 resume collector ====="
    HZ15_LOG="logs/hz15_jump_pages_resume_4_67_slow_${TS}.log"
    (
      export HZ15_RUN_ONCE=false
      export HZ15_TARGET_TOTAL=4000
      export HZ15_PAGE_START=4
      export HZ15_PAGE_END=67
      export HZ15_PAGE_SEQUENCE="4-67"
      export HZ15_ITEMS_PER_PAGE_LIMIT=60
      export HZ15_ITEM_SLEEP_MIN=7
      export HZ15_ITEM_SLEEP_MAX=14
      export HZ15_PAGE_SLEEP_MIN=120
      export HZ15_PAGE_SLEEP_MAX=190
