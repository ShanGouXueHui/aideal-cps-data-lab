#!/usr/bin/env bash
# Restart HZ14 all-product full collector with v3 skip-bad-SKU resilience.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ14 restart v3 skip bad SKU ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz14_v3_restart_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ14 v3 full collector ====="
    HZ14_LOG="logs/hz14_all_product_full_v3_4000_${TS}.log"
    (
      ...
      nohup .venv-browser/bin/python run/hz14_all_product_full_collector_v3.py > "$HZ14_LOG" 2>&1 &
