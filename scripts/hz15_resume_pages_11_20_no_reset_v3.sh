#!/usr/bin/env bash
# HZ15 segmented resume v3: pages 11..20, no reset if current page is already usable.
# Scope: JD Union 商品推广 / 全部商品 only.
# Fix: avoid resetting to pageNo=1 and clicking 商品推广/全部商品, which can trigger verification.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ15 resume pages 11..20 no-reset v3 ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz15_no_reset_11_20_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ15 no-reset segmented collector v3 ====="
    HZ15_LOG="logs/hz15_jump_pages_resume_11_20_no_reset_v3_${TS}.log"
    (
      ...
      nohup .venv-browser/bin/python run/hz15_jump_pages_collector_v3_no_reset.py > "$HZ15_LOG" 2>&1 &
