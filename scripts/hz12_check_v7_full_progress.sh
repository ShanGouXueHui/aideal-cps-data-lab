#!/usr/bin/env bash
# HZ12 v7 full collection progress checker.
...
#   cd ~/projects/aideal-cps-data-lab && git fetch origin main && git rebase origin/main && bash scripts/hz12_check_v7_full_progress.sh
...
if ! cd "$PROJECT_DIR"; then
  ...
  REPORT_JSON="reports/hz12z_product_all_v7_full_progress_latest.json"
  REPORT_MD="docs/ops/DL2_HZ12Z_PRODUCT_ALL_V7_FULL_PROGRESS.md"

  echo "===== HZ12 v7 full progress check ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
...
def events(path, limit=240):
    p = Path(path)
    out = []
    if not path or not p.exists():
        return out
    keep = {"PAGE_CANDIDATES", "ITEM_OK", "ITEM_FAIL", "STOP_REQUIRED", "TARGET_TOTAL_REACHED", "PRODUCT_NEXT_TEXT", "PRODUCT_NEXT_TEXT_FAIL", "PRODUCT_NEXT_TEXT_UNCHANGED_LIMIT", "FULL_CYCLE_DONE", "SLEEP_REFRESH_NOT_DUE", "STRICT_TITLE_CANDIDATES"}
...
report = {
    ...
    "phase": "HZ12Z v7 full 4000 progress check",
    "latest_log": latest_log,
    ...
    "missing": missing,
    "pagination": {"next_events_tail": len(next_events), "changed_next_events_tail": len(changed_next), "last_next": next_events[-1] if next_events else None},
    ...
    "decision": "If process is alive, stop=false, title missing=0, and dedup_sku keeps growing, continue. If target reached or worker sleeps, run final quality/dry-run report."
}
report_json.write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
md = []
md.append("# HZ12Z Product All V7 Full Progress")
md.append("")
...
print(json.dumps({"report": str(report_json), "counts": report["counts"], "missing": missing, "stop_exists": report["stop_exists"], "pagination": report["pagination"], "failures": report["failures"], "throughput": report["throughput"]}, ensure_ascii=False, indent=2))
PY

  git add "$REPORT_JSON" "$REPORT_MD"
  git commit -m "docs: add HZ12Z v7 full progress report" >/dev/null 2>&1 || true
