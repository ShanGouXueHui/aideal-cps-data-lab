#!/usr/bin/env bash
# HZ15 segmented resume v5: pages 11..20, no reset if current page is already usable.
# Fix: non-recursive strict page_info/risk patch.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ15 resume pages 11..20 no-reset v5 ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz15_no_reset_v5_11_20_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ15 no-reset segmented collector v5 ====="
    HZ15_LOG="logs/hz15_jump_pages_resume_11_20_no_reset_v5_${TS}.log"
    (
      ...
      nohup .venv-browser/bin/python run/hz15_jump_pages_collector_v5_no_reset.py > "$HZ15_LOG" 2>&1 &
