#!/usr/bin/env bash
# Patch HZ21 jsonl helper mismatch, run strict DOM-card SKU recovery smoke for page 49,
# and publish report to GitHub. No exit and no set -e are used.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ21 patch and strict-card-dom page49 smoke ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  echo "SERVER_TIME=$(date '+%Y-%m-%d %H:%M:%S %Z')"
  echo "HEAD_BEFORE=$(git rev-parse --short HEAD 2>/dev/null || true)"

  echo "===== patch HZ21 helper mismatch ====="
  python3 - <<'PY'
from pathlib import Path
p=Path('run/hz21_strict_card_dom_recover_page.py')
s=p.read_text(encoding='utf-8')
s=s.replace('rows=core.load_jsonl(core.LATEST)', 'rows=core.read_jsonl(core.LATEST)')
s=s.replace(
...
if not j.exists():
    latest_log=sorted(Path('logs').glob('hz21_strict_card_dom_page49_smoke_*.log'), key=lambda p:p.stat().st_mtime)[-1]
    md.write_text('# DL2 HZ21 Strict Card DOM Page49 Smoke\n\nNo JSON report found.\n\n```text\n'+latest_log.read_text(encoding='utf-8', errors='replace')[-8000:]+'\n```\n', encoding='utf-8')
else:
...
PY
  ...
  git add "$JSON" "$MD" "$LOG" run/hz21_strict_card_dom_recover_page.py scripts/hz21_patch_and_run_strict_card_dom_page49_smoke.sh data/import/hz_jd_union_all_product_full_links_latest.jsonl data/import/hz_jd_union_product_all_full_links_latest.jsonl run/hz14_all_product_full_report_latest.json run/hz14_all_product_full_state.json 2>/dev/null || true
