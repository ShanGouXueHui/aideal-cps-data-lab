#!/usr/bin/env bash
# HZ14 start official all-product full collector.
# Scope: 商品推广 / 全部商品 only.
# Pagination: Element UI .el-pagination button.btn-next.
# Quality: strict link rows; risk verification stops immediately.
# No exit is used.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ14 start all-product full 4000 ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  echo "HEAD_BEFORE=$(git rev-parse --short HEAD 2>/dev/null || true)"

  echo "===== stop old HZ collectors only, keep Chrome/noVNC ====="
  ...
  echo "===== backup HZ14 stop/state/report only; keep latest data ====="
  for f in \
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz14_full_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ14 full collector ====="
    HZ14_LOG="logs/hz14_all_product_full_4000_${TS}.log"
    (
      ...
      nohup .venv-browser/bin/python run/hz14_all_product_full_collector.py > "$HZ14_LOG" 2>&1 &
