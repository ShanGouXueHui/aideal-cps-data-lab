#!/usr/bin/env bash
# Run HZ21 strict safe-locator recovery for pages 53-57.
...
if ! cd "$PROJECT_DIR"; then
  ...
  LOG="logs/hz21_safe_pages_53_57_${TS}.log"
  MD="docs/ops/DL2_HZ21_SAFE_PAGES_53_57_SMOKE.md"
  SUMMARY_JSON="reports/hz21_safe_pages_53_57_latest.json"

  echo "===== HZ21 safe pages 53-57 smoke =====" | tee -a "$LOG"
  ...
  PAGES_OK=0
  PAGES_FAIL=0
  : > reports/hz21_safe_pages_53_57_per_page.jsonl

  for PAGE_NO in 53 54 55 56 57; do
...
Path('reports/hz21_safe_pages_53_57_per_page.jsonl').open('a',encoding='utf-8').write(json.dumps(row,ensure_ascii=False,sort_keys=True)+'\n')
...
from pathlib import Path
rows=[]
p=Path('reports/hz21_safe_pages_53_57_per_page.jsonl')
if p.exists():
  rows=[json.loads(x) for x in p.read_text(encoding='utf-8').splitlines() if x.strip()]
out={'pages':[53,54,55,56,57],'rows':rows,'total_ok':sum(int(r.get('total_ok') or 0) for r in rows),'total_fail':sum(int(r.get('total_fail') or 0) for r in rows),'last_known_sku_count': rows[-1].get('known_sku_count') if rows else None}
Path('reports/hz21_safe_pages_53_57_latest.json').write_text(json.dumps(out,ensure_ascii=False,indent=2,sort_keys=True),encoding='utf-8')
md=['# DL2 HZ21 Safe Pages 53-57 Smoke','', '```json', json.dumps(out,ensure_ascii=False,indent=2), '```']
Path('docs/ops/DL2_HZ21_SAFE_PAGES_53_57_SMOKE.md').write_text('\n'.join(md),encoding='utf-8')
print(json.dumps(out,ensure_ascii=False,sort_keys=True))
PY

  echo "===== commit reports/data/log =====" | tee -a "$LOG"
  git add reports/hz21_safe_pages_53_57_latest.json reports/hz21_safe_pages_53_57_per_page.jsonl reports/hz21_safe_page_*_latest.json reports/hz21_prepare_4000_pager_page*_latest.json docs/ops/DL2_HZ21_SAFE_PAGES_53_57_SMOKE.md "$LOG" data/import/hz_jd_union_all_product_full_links_latest.jsonl data/import/hz_jd_union_product_all_full_links_latest.jsonl run/hz14_all_product_full_report_latest.json run/hz14_all_product_full_state.json 2>/dev/null || true
  git commit -m "docs: publish HZ21 safe pages 53-57 smoke" >/dev/null 2>&1 || true
