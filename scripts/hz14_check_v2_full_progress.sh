#!/usr/bin/env bash
# HZ14 v2 official all-product full collector progress checker.
...
#   cd ~/projects/aideal-cps-data-lab && git fetch origin main && git rebase origin/main && bash scripts/hz14_check_v2_full_progress.sh
...
if ! cd "$PROJECT_DIR"; then
  ...
  LATEST_LOG="$(ls -t logs/hz14_all_product_full_v2_4000_*.log logs/hz14_all_product_full_4000_*.log 2>/dev/null | head -n 1 || true)"
  REPORT_JSON="reports/hz14_all_product_full_v2_progress_latest.json"
  REPORT_MD="docs/ops/DL2_HZ14_ALL_PRODUCT_FULL_V2_PROGRESS.md"

  echo "===== HZ14 v2 full progress check ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
...
report = {
    ...
    "phase": "HZ14 v2 official all-product full progress check",
    "latest_log": latest_log,
...
md = []
md.append("# HZ14 V2 Official All-Product Full Progress")
md.append("")
...
PY
  ...
  git commit -m "docs: add HZ14 v2 full progress report" >/dev/null 2>&1 || true
