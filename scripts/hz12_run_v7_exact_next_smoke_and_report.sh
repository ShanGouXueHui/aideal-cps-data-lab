#!/usr/bin/env bash
# HZ12 v7 exact-text next-page smoke runner.
...
#   cd ~/projects/aideal-cps-data-lab && git fetch origin main && git rebase origin/main && bash scripts/hz12_run_v7_exact_next_smoke_and_report.sh
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ12 v7 exact-next smoke start ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz12y_v7_smoke_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== run v7 exact-next smoke target 100 ====="
    SMOKE_LOG="logs/hz12y_product_all_v7_exact_next_smoke_${TS}.log"
    (
      set -a
      ...
      timeout 3600 .venv-browser/bin/python run/hz12_product_all_full_collector_v7.py
    ) > "$SMOKE_LOG" 2>&1
    SMOKE_RC=$?
  fi
...
def events(path):
    ...
    if not path or not p.exists():
        return out
    keep = {"STRICT_TITLE_CANDIDATES", "TITLE_ENRICHED_CANDIDATES_V3", "PAGE_CANDIDATES", "ITEM_OK", "ITEM_FAIL", "STOP_REQUIRED", "FULL_CYCLE_DONE", "TARGET_TOTAL_REACHED", "PRODUCT_NEXT_TEXT", "PRODUCT_NEXT_TEXT_FAIL", "PRODUCT_NEXT_TEXT_UNCHANGED_LIMIT"}
...
ev = events(log_path)
next_events = [x for x in ev if x.get("event") == "PRODUCT_NEXT_TEXT"]
...
page_candidate_events = [x for x in ev if x.get("event") == "PAGE_CANDIDATES"]
report = {
    "ts": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    "phase": "HZ12Y v7 strict-title exact-next smoke",
    ...
    "missing": missing,
    "pagination": {"next_events": len(next_events), "changed_next_events": len(changed_next), "last_next": next_events[-1] if next_events else None},
    ...
    "page_candidates": {"events": len(page_candidate_events), "last": page_candidate_events[-1] if page_candidate_events else None},
    ...
    "events_tail": ev,
    "log": log_path,
    "decision": "If ok reaches 100, title missing is 0, no STOP, and pagination changed at least once, start full v7 collection."
}
Path("reports/hz12y_product_all_v7_exact_next_smoke_latest.json").write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
md = []
md.append("# HZ12Y Product All V7 Exact Next Smoke")
md.append("")
...
md.append("")
Path("docs/ops/DL2_HZ12Y_PRODUCT_ALL_V7_EXACT_NEXT_SMOKE.md").write_text("\n".join(md), encoding="utf-8")
print(json.dumps({"report": "reports/hz12y_product_all_v7_exact_next_smoke_latest.json", "counts": report["counts"], "missing": missing, "stop_exists": report["stop_exists"], "pagination": report["pagination"], "failures": report["failures"], "strict_title": report["strict_title"]}, ensure_ascii=False, indent=2))
PY

  echo "===== commit and push report ====="
  git add reports/hz12y_product_all_v7_exact_next_smoke_latest.json docs/ops/DL2_HZ12Y_PRODUCT_ALL_V7_EXACT_NEXT_SMOKE.md
  git commit -m "docs: add HZ12Y v7 exact-next smoke report" >/dev/null 2>&1 || true
  ...
  echo "REPORT=reports/hz12y_product_all_v7_exact_next_smoke_latest.json"
