#!/usr/bin/env bash
# HZ14 v4 official all-product full collector progress checker.
# Scope: 商品推广 / 全部商品 only.
# Reads latest v4/v3/v2 logs, cumulative latest JSONL, STOP/state/runtime reports.
...
#   cd ~/projects/aideal-cps-data-lab && git fetch origin main && git rebase origin/main && bash scripts/hz14_check_v4_full_progress.sh
...
if ! cd "$PROJECT_DIR"; then
  ...
  LATEST_LOG="$(ls -t \
    logs/hz14_all_product_full_v4_4000_*.log \
    logs/hz14_all_product_full_v3_4000_*.log \
    logs/hz14_all_product_full_v2_4000_*.log \
    logs/hz14_all_product_full_4000_*.log 2>/dev/null | head -n 1 || true)"
  REPORT_JSON="reports/hz14_all_product_full_v4_progress_latest.json"
  REPORT_MD="docs/ops/DL2_HZ14_ALL_PRODUCT_FULL_V4_PROGRESS.md"

  echo "===== HZ14 v4 full progress check ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
...
def events(path, limit=420):
    p = Path(path)
    out = []
    if not path or not p.exists():
        return out
    keep = {"HZ14_ALL_PRODUCT_FULL_START", "HZ14_V4_BOOTSTRAP_MERGE", "PRODUCT_ALL_RESET", "PAGE_CANDIDATES", "ITEM_OK", "ITEM_FAIL", "ITEM_SKIP", "PAGE_SLEEP", "PAGE_NEXT", "STOP_REQUIRED", "TARGET_TOTAL_REACHED", "SLEEP_TARGET_TOTAL", "CYCLE_DONE"}
...
report = {
    ...
    "phase": "HZ14 v4 official all-product full cumulative progress check",
    "latest_log": latest_log,
    ...
    "decision": "Continue if process_alive=true, stop=false, missing core fields all zero, latest is cumulative, skips do not stop the job, and pagination continues. If STOP exists, inspect stop and do not restart automatically."
}
report_json.write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
md = []
md.append("# HZ14 V4 Official All-Product Full Cumulative Progress")
md.append("")
...
md.append(f"- bootstrap: {report['bootstrap']}")
...
print(json.dumps({"report": str(report_json), "process_alive": process_alive, "stop_exists": report["stop_exists"], "counts": report["counts"], "missing": missing, "bootstrap": report["bootstrap"], "pagination": report["pagination"], "failures": report["failures"], "throughput": report["throughput"]}, ensure_ascii=False, indent=2))
PY

  git add "$REPORT_JSON" "$REPORT_MD"
  git commit -m "docs: add HZ14 v4 cumulative progress report" >/dev/null 2>&1 || true
