#!/usr/bin/env bash
# Restart HZ14 all-product full collector with v2 state compatibility fix.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ14 restart v2 after state fix ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  echo "===== backup STOP/state/report; keep latest data ====="
  for f in \
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz14_v2_restart_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ14 v2 full collector ====="
    HZ14_LOG="logs/hz14_all_product_full_v2_4000_${TS}.log"
    (
      ...
      nohup .venv-browser/bin/python run/hz14_all_product_full_collector_v2.py > "$HZ14_LOG" 2>&1 &
