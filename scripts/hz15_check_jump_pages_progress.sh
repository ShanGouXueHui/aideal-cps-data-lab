#!/usr/bin/env bash
# HZ15 jump-pages collector progress checker.
# Scope: 商品推广 / 全部商品 only.
# Reads latest HZ15 logs and cumulative latest JSONL.
# No exit is used.
...
if ! cd "$PROJECT_DIR"; then
  ...
  LATEST_LOG="$(ls -t logs/hz15_jump_pages_1_67_*.log logs/hz14_all_product_full_v4_4000_*.log logs/hz14_all_product_full_v3_4000_*.log 2>/dev/null | head -n 1 || true)"
  REPORT_JSON="reports/hz15_jump_pages_progress_latest.json"
  REPORT_MD="docs/ops/DL2_HZ15_JUMP_PAGES_PROGRESS.md"

  echo "===== HZ15 jump-pages progress check ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
...
def events(path, limit=520):
    p = Path(path)
    out = []
    if not path or not p.exists():
        return out
    keep = {'HZ15_JUMP_PAGES_START', 'HZ14_V4_BOOTSTRAP_MERGE', 'PRODUCT_ALL_RESET', 'PAGE_JUMP', 'PAGE_JUMP_SLEEP', 'PAGE_CANDIDATES', 'ITEM_OK', 'ITEM_FAIL', 'ITEM_SKIP', 'STOP_REQUIRED', 'TARGET_TOTAL_REACHED', 'SLEEP_TARGET_TOTAL', 'CYCLE_DONE'}
...
report = {
    ...
    'phase': 'HZ15 jump-pages cumulative progress check',
    'latest_log': latest_log,
    ...
    'decision': 'Continue if process_alive=true, stop=false, jump ok, latest cumulative grows, skips do not stop the job, and missing core fields remain zero.'
}
report_json.write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True), encoding='utf-8')
md = []
md.append('# HZ15 Jump Pages Progress')
md.append('')
...
PY
  ...
  git commit -m "docs: add HZ15 jump-pages progress report" >/dev/null 2>&1 || true
