#!/usr/bin/env bash
# HZ15 segmented resume v4: pages 11..20, no reset if current page is already usable.
# Fix: explicit module aliases for HZ15 wrapper and HZ14 core.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ15 resume pages 11..20 no-reset v4 ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz15_no_reset_v4_11_20_${TS}" || true
    fi
  done
  ...
  .venv-browser/bin/python -m py_compile \
    ...
    run/hz15_jump_pages_collector_v2.py \
    run/hz15_jump_pages_collector_v3_no_reset.py \
    run/hz15_jump_pages_collector_v4_no_reset.py
  STATIC_RC=$?
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ15 no-reset segmented collector v4 ====="
    HZ15_LOG="logs/hz15_jump_pages_resume_11_20_no_reset_v4_${TS}.log"
    (
      ...
      nohup .venv-browser/bin/python run/hz15_jump_pages_collector_v4_no_reset.py > "$HZ15_LOG" 2>&1 &
