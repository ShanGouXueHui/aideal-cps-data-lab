#!/usr/bin/env bash
# HZ15 segmented resume v6: pages 11..20, strict 4000 all-product page gate.
# Scope: JD Union 商品推广 / 全部商品 only.
# It will not reset/click automatically. Current browser page must already show 共4000条 + 60 items.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ15 resume pages 11..20 no-reset strict-4000 v6 ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz15_no_reset_v6_11_20_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ15 strict-4000 segmented collector v6 ====="
    HZ15_LOG="logs/hz15_jump_pages_resume_11_20_no_reset_v6_strict_4000_${TS}.log"
    (
