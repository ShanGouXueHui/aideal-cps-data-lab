#!/usr/bin/env bash
# HZ15 ultra-slow segmented resume v2: pages 11..20.
# Fix: strict risk detection; do not treat ordinary page text "风险" as verification.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ15 resume pages 11..20 ultra-slow v2 ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz15_resume_11_20_v2_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ15 ultra-slow segmented collector v2 ====="
    HZ15_LOG="logs/hz15_jump_pages_resume_11_20_ultraslow_v2_${TS}.log"
    (
      ...
      nohup .venv-browser/bin/python run/hz15_jump_pages_collector_v2.py > "$HZ15_LOG" 2>&1 &
