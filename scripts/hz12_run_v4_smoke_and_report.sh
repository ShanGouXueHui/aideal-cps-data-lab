#!/usr/bin/env bash
# HZ12 v4 UI-pagination smoke runner.
# Usage:
#   cd ~/projects/aideal-cps-data-lab || exit 1
#   git fetch origin main && git rebase origin/main
#   bash scripts/hz12_run_v4_smoke_and_report.sh
#
# Purpose:
# - Validate v4 UI pagination and title-rich product_all collection.
# - Run a bounded smoke target of 80 SKU so it must cross page 1.
# - Generate compact report and push it to GitHub.
# - Does not close Chrome. Does not write production DB.
...
for f in \
 ...
 do
  if [ -e "$f" ]; then
    mv -v "$f" "backups/$(basename "$f").before_hz12s_v4_smoke_${TS}" || true
  fi
 done
...
echo "===== run v4 smoke target 80 ====="
SMOKE_LOG="logs/hz12s_product_all_v4_ui_pagination_smoke_${TS}.log"
(
  set -a
  ...
  timeout 3600 .venv-browser/bin/python run/hz12_product_all_full_collector_v4.py
) > "$SMOKE_LOG" 2>&1
SMOKE_RC=$?
...
def events(path):
    ...
    if not p.exists():
        return out
    keep = {
        "TITLE_ENRICHED_CANDIDATES_V3", "PAGE_CANDIDATES", "ITEM_OK", "ITEM_FAIL",
        "STOP_REQUIRED", "FULL_CYCLE_DONE", "TARGET_TOTAL_REACHED", "PRODUCT_NEXT_PAGE",
        "PRODUCT_NEXT_PAGE_FAIL", "PRODUCT_NEXT_UNCHANGED_LIMIT"
    }
...
report = {
    ...
    "phase": "HZ12S v4 product_all UI pagination smoke",
    ...
    "sample_last_20": [
        {"page_no": x.get("page_no"), "sku": x.get("sku"), "short_url": x.get("short_url"), "title": (x.get("title") or "")[:100], "price": x.get("price"), "commission_rate": x.get("commission_rate"), "estimated_income": x.get("estimated_income")}
        for x in ok[-20:]
    ],
    "events_tail": ev,
    "log": log_path,
    "decision": "If ok reaches 80, title missing is 0, no STOP, and pagination changed at least once, start full v4 collection."
}
Path("reports/hz12s_product_all_v4_pagination_smoke_latest.json").write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
md = []
md.append("# HZ12S Product All V4 Pagination Smoke")
md.append("")
...
md.append("")
Path("docs/ops/DL2_HZ12S_PRODUCT_ALL_V4_PAGINATION_SMOKE.md").write_text("\n".join(md), encoding="utf-8")
print(json.dumps({"report": "reports/hz12s_product_all_v4_pagination_smoke_latest.json", "counts": report["counts"], "missing": missing, "stop_exists": report["stop_exists"], "pagination": report["pagination"], "failures": report["failures"]}, ensure_ascii=False, indent=2))
PY

echo "===== commit and push report ====="
git add reports/hz12s_product_all_v4_pagination_smoke_latest.json docs/ops/DL2_HZ12S_PRODUCT_ALL_V4_PAGINATION_SMOKE.md
git commit -m "docs: add HZ12S v4 pagination smoke report" >/dev/null 2>&1 || true
...
echo "REPORT=reports/hz12s_product_all_v4_pagination_smoke_latest.json"
