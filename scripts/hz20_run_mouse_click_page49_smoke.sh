#!/usr/bin/env bash
# Run HZ20 physical mouse-click recovery smoke for page 49 and publish report to GitHub.
...
if ! cd "$PROJECT_DIR"; then
  ...
  LOG="logs/hz20_mouse_click_page49_smoke_${TS}.log"
  JSON="reports/hz20_mouse_click_recover_latest.json"
  MD="docs/ops/DL2_HZ20_MOUSE_CLICK_PAGE49_SMOKE.md"

  echo "===== HZ20 mouse-click page49 smoke ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  sleep 3
  pgrep -af "hz15_daytime_autostart_supervisor_40_67|hz17_recover|hz18_card|hz20_mouse|chrome.*19228" | head -n 100 || true

  echo "===== static check ====="
  .venv-browser/bin/python -m py_compile run/hz20_mouse_click_recover_page.py
  STATIC_RC=$?

  echo "===== run HZ20 mouse-click smoke ====="
  HZ20_PAGE_SEQUENCE=49 HZ20_LIMIT=15 HZ20_WAIT=10 HZ20_FAIL_FUSE=6 HZ20_ITEM_SLEEP_MIN=1 HZ20_ITEM_SLEEP_MAX=3 .venv-browser/bin/python run/hz20_mouse_click_recover_page.py > "$LOG" 2>&1
  HZ20_RC=$?
...
j=Path('reports/hz20_mouse_click_recover_latest.json')
md=Path('docs/ops/DL2_HZ20_MOUSE_CLICK_PAGE49_SMOKE.md')
if not j.exists():
    md.write_text('# DL2 HZ20 Mouse Click Page49 Smoke\n\nNo JSON report found.\n', encoding='utf-8')
else:
    x=json.loads(j.read_text(encoding='utf-8'))
    lines=[]
    lines.append('# DL2 HZ20 Mouse Click Page49 Smoke')
    lines.append('')
    ...
    lines.append('```json')
    lines.append(json.dumps((x.get('results') or [])[-20:], ensure_ascii=False, indent=2)[:18000])
    lines.append('```')
    md.write_text('\n'.join(lines), encoding='utf-8')
PY

  echo "===== commit reports/data/log ====="
  git add "$JSON" "$MD" "$LOG" run/hz20_mouse_click_recover_page.py scripts/hz20_run_mouse_click_page49_smoke.sh data/import/hz_jd_union_all_product_full_links_latest.jsonl data/import/hz_jd_union_product_all_full_links_latest.jsonl run/hz14_all_product_full_report_latest.json run/hz14_all_product_full_state.json
  git commit -m "docs: publish HZ20 mouse click page49 smoke" >/dev/null 2>&1 || true
