#!/usr/bin/env bash
# HZ15 no-reset v6 strict-4000 progress checker.
...
if ! cd "$PROJECT_DIR"; then
  ...
  LATEST_LOG="$(ls -t \
    logs/hz15_jump_pages_resume_11_20_no_reset_v6_strict_4000_*.log \
    logs/hz15_jump_pages_resume_11_20_no_reset_v5_*.log \
    logs/hz15_jump_pages_resume_11_20_no_reset_v4_*.log 2>/dev/null | head -n 1 || true)"
  REPORT_JSON="reports/hz15_no_reset_v6_progress_latest.json"
  REPORT_MD="docs/ops/DL2_HZ15_NO_RESET_V6_PROGRESS.md"

  echo "===== HZ15 no-reset v6 strict-4000 progress check ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
...
report={
 ...
 'phase':'HZ15 no-reset v6 strict-4000 progress check',
 ...
 'decision':'Continue if strict 4000 gate passed, process_alive=true, stop=false, PAGE_JUMP/ITEM_OK appear, and dedup_sku grows beyond 967. If manual_product_all_4000_required, put browser on 商品推广/全部商品 共4000条 and rerun.'
}
Path('$REPORT_JSON').write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True), encoding='utf-8')
Path('$REPORT_MD').write_text('# HZ15 No-Reset V6 Strict-4000 Progress\n\n' + f"- Generated at: {report['ts']}\n- process_alive: {report['process_alive']}\n- stop_exists: {report['stop_exists']}\n- counts: {report['counts']}\n- missing: {missing}\n- event_count: {report['event_count']}\n- interesting_event_count: {report['interesting_event_count']}\n", encoding='utf-8')
...
PY
  ...
  git commit -m "docs: add HZ15 no-reset v6 progress report" >/dev/null 2>&1 || true
