#!/usr/bin/env bash
# HZ12 v4 UI-pagination smoke runner, safe shell version.
...
#   cd ~/projects/aideal-cps-data-lab && git fetch origin main && git rebase origin/main && bash scripts/hz12_run_v4_smoke_and_report_safe.sh
...
def events(path):
    ...
    if not path or not p.exists():
        return out
    keep = {"TITLE_ENRICHED_CANDIDATES_V3", "PAGE_CANDIDATES", "ITEM_OK", "ITEM_FAIL", "STOP_REQUIRED", "FULL_CYCLE_DONE", "TARGET_TOTAL_REACHED", "PRODUCT_NEXT_PAGE", "PRODUCT_NEXT_PAGE_FAIL", "PRODUCT_NEXT_UNCHANGED_LIMIT"}
...
report = {
    ...
    "phase": "HZ12S v4 product_all UI pagination smoke safe",
...
md = []
md.append("# HZ12S Product All V4 Pagination Smoke Safe")
md.append("")
