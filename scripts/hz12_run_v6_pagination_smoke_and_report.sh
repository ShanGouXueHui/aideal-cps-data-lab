#!/usr/bin/env bash
# HZ12 v6 strict-title + robust-pager smoke runner.
...
#   cd ~/projects/aideal-cps-data-lab && git fetch origin main && git rebase origin/main && bash scripts/hz12_run_v6_pagination_smoke_and_report.sh
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ12 v6 pagination smoke start ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz12u_v6_smoke_${TS}" || true
    fi
  done
  ...
  .venv-browser/bin/python -m py_compile \
    ...
    run/hz12_product_all_full_collector_v6.py
  STATIC_RC=$?
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== run v6 pagination smoke target 100 ====="
    SMOKE_LOG="logs/hz12u_product_all_v6_pagination_smoke_${TS}.log"
    (
      set -a
      ...
      timeout 3600 .venv-browser/bin/python run/hz12_product_all_full_collector_v6.py
    ) > "$SMOKE_LOG" 2>&1
    SMOKE_RC=$?
  fi
...
report = {
    ...
    "phase": "HZ12U v6 strict-title robust-pager smoke",
    ...
    "events_tail": ev,
    "log": log_path,
    "decision": "If ok reaches 100, title missing is 0, no STOP, and pagination changed at least once, start full v6 collection."
}
Path("reports/hz12u_product_all_v6_pagination_smoke_latest.json").write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
md = []
md.append("# HZ12U Product All V6 Robust Pager Smoke")
md.append("")
...
md.append("")
Path("docs/ops/DL2_HZ12U_PRODUCT_ALL_V6_PAGINATION_SMOKE.md").write_text("\n".join(md), encoding="utf-8")
print(json.dumps({"report": "reports/hz12u_product_all_v6_pagination_smoke_latest.json", "counts": report["counts"], "missing": missing, "stop_exists": report["stop_exists"], "pagination": report["pagination"], "failures": report["failures"], "strict_title": report["strict_title"]}, ensure_ascii=False, indent=2))
PY

  echo "===== commit and push report ====="
  git add reports/hz12u_product_all_v6_pagination_smoke_latest.json docs/ops/DL2_HZ12U_PRODUCT_ALL_V6_PAGINATION_SMOKE.md
  git commit -m "docs: add HZ12U v6 robust-pager smoke report" >/dev/null 2>&1 || true
  ...
  echo "REPORT=reports/hz12u_product_all_v6_pagination_smoke_latest.json"
