#!/usr/bin/env bash
# HZ15 segmented resume v6: pages 28..30, strict 4000 all-product page gate.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ15 resume pages 28..30 no-reset strict-4000 v6 ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz15_no_reset_v6_28_30_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ15 strict-4000 segmented collector v6 pages 28..30 ====="
    HZ15_LOG="logs/hz15_jump_pages_resume_28_30_no_reset_v6_strict_4000_${TS}.log"
    (
      export HZ15_RUN_ONCE=true
      export HZ15_TARGET_TOTAL=4000
      export HZ15_PAGE_START=28
      export HZ15_PAGE_END=30
      export HZ15_PAGE_SEQUENCE="28-30"
      export HZ15_ITEMS_PER_PAGE_LIMIT=60
      export HZ15_ITEM_SLEEP_MIN=10
      export HZ15_ITEM_SLEEP_MAX=22
      export HZ15_PAGE_SLEEP_MIN=360
      export HZ15_PAGE_SLEEP_MAX=620
