#!/usr/bin/env bash
# HZ15 segmented resume v6: pages 21..30, strict 4000 all-product page gate.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ15 resume pages 21..30 no-reset strict-4000 v6 ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz15_no_reset_v6_21_30_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ15 strict-4000 segmented collector v6 pages 21..30 ====="
    HZ15_LOG="logs/hz15_jump_pages_resume_21_30_no_reset_v6_strict_4000_${TS}.log"
    (
      export HZ15_RUN_ONCE=true
      export HZ15_TARGET_TOTAL=4000
      export HZ15_PAGE_START=21
      export HZ15_PAGE_END=30
      export HZ15_PAGE_SEQUENCE="21-30"
