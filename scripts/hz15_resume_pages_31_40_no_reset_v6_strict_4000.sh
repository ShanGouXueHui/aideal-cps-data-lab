#!/usr/bin/env bash
# HZ15 segmented resume v6: pages 31..40, strict 4000 all-product page gate.
...
if ! cd "$PROJECT_DIR"; then
  ...
  echo "===== HZ15 resume pages 31..40 no-reset strict-4000 v6 ====="
  echo "PWD=$(pwd)"
  echo "USER=$(whoami)"
  ...
  do
    if [ -e "$f" ]; then
      mv -v "$f" "backups/$(basename "$f").before_hz15_no_reset_v6_31_40_${TS}" || true
    fi
  done
  ...
  if [ "$STATIC_RC" != "0" ]; then
    ...
    echo "===== start background HZ15 strict-4000 segmented collector v6 pages 31..40 ====="
    HZ15_LOG="logs/hz15_jump_pages_resume_31_40_no_reset_v6_strict_4000_${TS}.log"
    (
      export HZ15_RUN_ONCE=true
      export HZ15_TARGET_TOTAL=4000
      export HZ15_PAGE_START=31
      export HZ15_PAGE_END=40
      export HZ15_PAGE_SEQUENCE="31-40"
