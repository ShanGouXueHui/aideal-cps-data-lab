-- AIdeal CPS Data Lab commission data schema V1
-- Draft only. Do not execute in production before HZ23 observation acceptance,
-- backup, migration rehearsal, and explicit review.

CREATE DATABASE IF NOT EXISTS aideal_cps_data_lab
  CHARACTER SET utf8mb4
  COLLATE utf8mb4_unicode_ci;

USE aideal_cps_data_lab;
...
CREATE TABLE IF NOT EXISTS commission_products (
  ...
  commission_rate DECIMAL(8,4) NULL COMMENT 'percentage number; 12.5000 means 12.5%',
  ...
  CONSTRAINT ck_commission_products_status
    CHECK (status IN ('active', 'inactive', 'quarantined')),
  CONSTRAINT ck_commission_products_page
    CHECK (source_page_no IS NULL OR source_page_no BETWEEN 1 AND 67),
  CONSTRAINT ck_commission_products_nonnegative
    CHECK (
      price >= 0
...
CREATE TABLE IF NOT EXISTS commission_refresh_runs (
  ...
  CONSTRAINT ck_commission_refresh_runs_type
    CHECK (run_type IN ('probe', 'full', 'backfill', 'retry', 'publish')),
  CONSTRAINT ck_commission_refresh_runs_status
...
CREATE TABLE IF NOT EXISTS commission_product_history (
  ...
  CONSTRAINT ck_commission_product_history_type
    CHECK (change_type IN ('insert', 'update', 'status', 'link'))
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS commission_publish_versions (
  ...
  CONSTRAINT ck_commission_publish_status
...
-- User creation and GRANT are intentionally omitted.
-- Generate credentials on the server and grant only:
--   SELECT on v_published_commission_products
--   SELECT on commission_publish_versions
-- Never commit passwords or public 3306 exposure configuration.
