# 佣金商品 MySQL 同步架构 V1

更新日期：2026-06-14

状态：观察期后的目标商用架构。观察期内可以完成准备，但不得在 HZ23 商用验收前切换正式数据源。

## 1. 决策摘要

当前 Data Lab 仍以 JSONL、state、history 和 report 保存数据，尚未建设正式 MySQL 主库。

HZ23 观察通过后：

1. Data Lab 建立本地 MySQL 数据库 `aideal_cps_data_lab`；
2. 可信佣金商品回填到 `commission_products`；
3. HZ23 事务性更新 MySQL；
4. JSONL 继续作为快照、审计和灾备；
5. AIdeal CPS 定时通过短生命周期 SSH Tunnel 读取只读视图；
6. AIdeal CPS 将数据 upsert 到自己的本地 MySQL `aideal_cps.products`；
7. 用户请求只访问 AIdeal CPS 本地库。

不开放公网 MySQL 3306，不使用长期常驻 Tunnel，不允许用户请求实时查询 Data Lab。

## 2. 访问拓扑

两台机器均在杭州，但当前没有确认可用的同 VPC 私网地址和安全组互通信息，因此 V1 使用定时任务临时建立 Tunnel：

```text
AIdeal CPS deploy@8.136.28.6
127.0.0.1:13306
        |
        | ssh -L，任务开始时建立
        v
cpsdata@121.41.111.36
127.0.0.1:3306
```

同步结束立即关闭 Tunnel。后续如确认同 VPC 私网可用，只替换 Transport，不改变数据表、Repository 和同步语义。

## 3. 连接信息约定

### SSH

```text
SSH_HOST=121.41.111.36
SSH_USER=cpsdata
SSH_KEY=/home/deploy/.ssh/aideal_data_lab_ro_ed25519
SSH_SOURCE_SERVER=8.136.28.6
```

### Tunnel 后 MySQL

```text
DATA_LAB_DB_HOST=127.0.0.1
DATA_LAB_DB_PORT=13306
DATA_LAB_DB_NAME=aideal_cps_data_lab
DATA_LAB_DB_USER=aideal_cps_ro
DATA_LAB_DB_PASSWORD=<仅保存在 AIdeal CPS 服务器 Secret/.env>
```

Data Lab 本机：

```text
MYSQL_BIND_ADDRESS=127.0.0.1
MYSQL_PORT=3306
```

密码、私钥和真实 DSN 不进入 GitHub。

## 4. 数据表

### 4.1 `commission_products`

| 字段 | 类型 | 约束/说明 |
|---|---|---|
| `id` | BIGINT | PK, auto increment |
| `jd_sku_id` | VARCHAR(64) | UNIQUE, NOT NULL |
| `title` | VARCHAR(512) | NOT NULL |
| `description` | TEXT | NULL |
| `item_url` | VARCHAR(500) | 京东详情页 |
| `promotion_url` | VARCHAR(500) | 京东联盟推广链接，NOT NULL |
| `short_url` | VARCHAR(500) | 兼容字段 |
| `long_url` | TEXT | NULL |
| `qr_url` | TEXT | NULL |
| `jd_command` | TEXT | NULL |
| `image_url` | VARCHAR(1000) | NOT NULL |
| `category_name` | VARCHAR(128) | NULL |
| `shop_name` | VARCHAR(255) | NULL |
| `price` | DECIMAL(12,2) | NOT NULL |
| `coupon_price` | DECIMAL(12,2) | NULL |
| `commission_rate` | DECIMAL(8,4) | 12.5000 表示 12.5% |
| `estimated_commission` | DECIMAL(12,2) | NULL |
| `sales_volume` | BIGINT | NULL |
| `coupon_info` | VARCHAR(512) | NULL |
| `status` | VARCHAR(32) | active/inactive/quarantined |
| `is_published` | TINYINT(1) | 商用发布门禁 |
| `source_page_no` | SMALLINT | 1-67 |
| `source_round_id` | VARCHAR(64) | 最近轮次 |
| `source_run_id` | VARCHAR(64) | 最近 run |
| `source_payload_hash` | CHAR(64) | 业务字段 hash |
| `catalog_change_count` | INT | 字段变化次数 |
| `link_created_at` | DATETIME(6) | NULL |
| `link_expire_at` | DATETIME(6) | NULL |
| `refresh_due_at` | DATETIME(6) | NULL |
| `first_seen_at` | DATETIME(6) | NOT NULL |
| `last_checked_at` | DATETIME(6) | NOT NULL |
| `last_seen_at` | DATETIME(6) | NOT NULL |
| `published_at` | DATETIME(6) | NULL |
| `created_at` | DATETIME(6) | NOT NULL |
| `updated_at` | DATETIME(6) | NOT NULL |

索引：

```text
UNIQUE(jd_sku_id)
INDEX(is_published, status)
INDEX(updated_at, id)
INDEX(refresh_due_at)
INDEX(source_round_id)
INDEX(last_seen_at)
```

### 4.2 `commission_refresh_runs`

记录 probe/full/backfill/retry/publish 轮次，包括状态、页面范围、完成页、未完成页、扫描数量、新增/变化/未变化、领链成功/失败、风险信号、停止原因和开始结束时间。

### 4.3 `commission_product_history`

只保存业务字段变化，不为纯时间戳刷新写历史。核心字段：`jd_sku_id`、`round_id`、`change_type`、before/after payload、before/after hash、`changed_at`。

### 4.4 `commission_publish_versions`

记录对外发布版本：`publish_version`、`round_id`、candidate/published/revoked 状态、行数、JSONL SHA-256、schema version 和发布时间。

完整建表草案：

```text
docs/architecture/commission_data_mysql_v1.sql
```

## 5. 对外只读视图

AIdeal CPS 只查询：

```sql
CREATE VIEW v_published_commission_products AS
SELECT
    jd_sku_id,
    title,
    description,
    item_url,
    promotion_url,
    short_url,
    image_url,
    category_name,
    shop_name,
    price,
    coupon_price,
    commission_rate,
    estimated_commission,
    sales_volume,
    coupon_info,
    status,
    source_page_no,
    source_round_id,
    source_payload_hash,
    catalog_change_count,
    link_created_at,
    link_expire_at,
    refresh_due_at,
    last_checked_at,
    last_seen_at,
    published_at,
    updated_at
FROM commission_products
WHERE is_published = 1
  AND status = 'active';
```

只读用户仅授予：

```text
SELECT on aideal_cps_data_lab.v_published_commission_products
SELECT on aideal_cps_data_lab.commission_publish_versions
```

禁止 INSERT、UPDATE、DELETE、DDL，禁止授予 `*.*`。

## 6. 写入语义

### 新商品

- 按 `jd_sku_id` insert；
- 必填字段缺失进入 quarantined，不发布；
- promotion_url 未验证不得 `is_published=1`。

### 业务字段变化

- 比较规范化 payload hash；
- hash 改变才更新业务字段并写 history；
- 增加 `catalog_change_count`；
- 重新评估发布资格。

### 未变化

只更新：

```text
last_checked_at
last_seen_at
source_round_id
source_run_id
updated_at
```

不写 history。

### 商品消失

- 单次完整轮次未出现不下架；
- 连续两次完整成功轮次未出现，设置 inactive 和 `is_published=0`；
- 不物理删除。

## 7. JSONL 回填 MySQL

迁移要求：

1. 读取可信 latest JSONL；
2. 按 SKU 去重；
3. 仅接受 `status=ok` 且推广链接有效；
4. 金额使用 Decimal；
5. 佣金率去掉 `%` 后存 DECIMAL；
6. 计算 payload hash；
7. staging 校验；
8. 单事务 upsert；
9. 输出 inserted/updated/unchanged/rejected；
10. 重复执行幂等；
11. 失败不破坏 JSONL 源；
12. 抽样验证短链、SKU、价格和佣金。

## 8. AIdeal CPS 同步流程

当前规模约 2.4k-4k，V1 采用全量读取 + payload hash upsert，不提前引入复杂 CDC。

1. 获取同步锁；
2. 建立短生命周期 SSH Tunnel；
3. 查询最新 published version；
4. version 未变化则关闭 Tunnel 并退出；
5. 查询只读视图；
6. 本地 staging 校验唯一 SKU、必填字段和金额；
7. 单事务 upsert 到 `aideal_cps.products`；
8. promotion_url 写入 `products.promotion_url`，兼容期同时写 `product_url`；
9. 更新同步 run；
10. 关闭 Tunnel；
11. 失败时保留上一版本本地数据。

用户请求永远只读本地 `aideal_cps.products`。

## 9. 安全要求

SSH Key：

- 只允许来源 `8.136.28.6`；
- no-agent-forwarding；
- no-X11-forwarding；
- no-pty；
- 仅允许转发到 `127.0.0.1:3306`；
- 私钥权限 600；
- known_hosts 固定主机指纹。

MySQL：

- bind 127.0.0.1；
- writer 和 readonly 用户分离；
- 最小权限；
- 高强度随机密码；
- 密码仅存服务器 Secret/.env；
- 日志不得打印密码；
- 定期备份和恢复演练。

## 10. 配置变量

Data Lab：

```text
DATA_LAB_DATABASE_URL=mysql+pymysql://<writer>@127.0.0.1:3306/aideal_cps_data_lab
DATA_LAB_DB_WRITE_ENABLED=false
DATA_LAB_DB_DUAL_WRITE_ENABLED=false
DATA_LAB_PUBLISH_ENABLED=false
```

AIdeal CPS：

```text
DATA_LAB_SSH_HOST=121.41.111.36
DATA_LAB_SSH_USER=cpsdata
DATA_LAB_SSH_KEY=/home/deploy/.ssh/aideal_data_lab_ro_ed25519
DATA_LAB_TUNNEL_LOCAL_PORT=13306
DATA_LAB_DB_HOST=127.0.0.1
DATA_LAB_DB_PORT=13306
DATA_LAB_DB_NAME=aideal_cps_data_lab
DATA_LAB_DB_USER=aideal_cps_ro
DATA_LAB_DB_PASSWORD=<secret>
DATA_LAB_SYNC_ENABLED=false
```

默认所有写入、发布和同步开关为 false，验收后逐级打开。

## 11. 实施顺序

### 观察期准备

1. 增加 MySQL 依赖和配置对象；
2. 建 migration/DDL；
3. 建 domain model 和 Repository 接口；
4. 实现 MySQL Repository；
5. 实现 JSONL 回填器；
6. 实现一致性验证器；
7. 测试 migration upgrade/downgrade；
8. 建 AIdeal CPS Tunnel 和只读同步代码，但保持 disabled；
9. 不改变现有 HZ23 运行路径。

### HZ23 验收通过

1. 创建正式库及 writer/readonly 用户；
2. 执行回填；
3. 一致性检查；
4. 开启 Data Lab dual-write；
5. 观察一个完整刷新轮次；
6. MySQL 升为主写，JSONL 保留快照；
7. 发布只读视图；
8. AIdeal CPS dry-run 同步；
9. 灰度启用；
10. 正式商用。

## 12. 验收标准

- migration 可升级和回滚；
- 可信 SKU 回填数量与 JSONL 去重数量一致；
- `jd_sku_id` 无重复；
- published 记录必填字段完整；
- promotion_url 为可信京东推广域名；
- 金额和佣金正确 Decimal 化；
- 回填幂等；
- dual-write 一致性通过；
- AIdeal CPS 同步失败不影响旧数据；
- 3306 未暴露公网；
- Secret 未进入 GitHub。
