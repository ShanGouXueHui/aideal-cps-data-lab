...
class PythonAssignmentTests(unittest.TestCase):
    ...
    def test_runtime_local_reassignment_is_not_configuration_override(self):
        tree = ast.parse("def work():\n    count = 0\n    count = count + 1\n")
        self.assertEqual([], assignment_findings(tree, Path("sample.py")))

    def test_function_configuration_reassignment_is_blocked(self):
        tree = ast.parse("def work():\n    timeout_config = 1\n    timeout_config = 2\n")
        self.assertIn(
    ...
    def test_configuration_reassignment_inside_control_flow_is_blocked(self):
        tree = ast.parse(
            "def work(flag):\n    timeout_config = 1\n    if flag:\n        timeout_config = 2\n"
        )
        self.assertIn(
...
class ShellAssignmentTests(unittest.TestCase):
    def test_shell_duplicate_config_and_long_function(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            path = Path("sample.sh")
            body = ["#!/usr/bin/env bash", "PORT=1", "PORT=2", "work() {"]
            body.extend(f"  value_{index}={index}" for index in range(81))
            body.append("}")
            (root / path).write_text("\n".join(body) + "\n", encoding="utf-8")
            findings, _, _ = scan_shell(root, path, SETTINGS)
        found = categories(findings)
        self.assertIn("duplicate_constant_assignment", found)
        self.assertIn("long_function", found)
        self.assertNotIn("shell_syntax", found)

    def test_shell_runtime_local_reassignment_is_allowed(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            path = Path("sample.sh")
            (root / path).write_text(
            ...
            findings, _, _ = scan_shell(root, path, SETTINGS)
        self.assertNotIn("duplicate_assignment", categories(findings))
