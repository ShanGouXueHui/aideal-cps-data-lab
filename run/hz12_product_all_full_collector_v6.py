#!/usr/bin/env python3
"""HZ12 product_all v6 runner.

Fixes v4/v5 pager issue: previous next-page selector could click a huge wrapper
whose text contained 下一页, so the page did not actually advance. v6 keeps strict-title
quality gate from v5 and overrides the UI pager to click only small, visible, pager-like
next controls.
"""
...
def click_product_all_next_page_v6(page, state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    click_res = page.evaluate(
        ...
        () => {
          ...
          const mapped = all.map((el, idx) => {
            ...
            const visible = r.width > 0 && r.height > 0 && r.top >= -120 && r.left >= -80 && r.top <= window.innerHeight + 180;
            const small = r.width <= 120 && r.height <= 80;
            const pagerLike =
              cls.includes('btn-next') || cls === 'next' || cls.includes(' next') ||
              cls.includes('el-pager') || cls.includes('pagination') || role === 'button' ||
              aria.includes('下一页') || aria.toLowerCase().includes('next');
            const exactText = txt === '下一页' || txt === '>' || txt === '›';
            return {el, idx, txt, cls, aria, role, disabled, visible, small, pagerLike, exactText, rect:{x:r.x,y:r.y,w:r.width,h:r.height,cx:r.x+r.width/2,cy:r.y+r.height/2}};
          });

          let candidates = mapped.filter(x => x.visible && !x.disabled && x.small && (x.exactText || x.pagerLike));

          // Exclude large content wrappers and any node whose text is actually the whole page.
          candidates = candidates.filter(x => x.txt.length <= 12);

          candidates.sort((a,b) => {
            const ae = a.txt === '下一页' ? 0 : (a.cls.includes('next') ? 1 : 2);
            const be = b.txt === '下一页' ? 0 : (b.cls.includes('next') ? 1 : 2);
          ...
          if (!candidates.length) {
            return {
              ok:false,
              reason:'next_not_found_small_control',
              samples:mapped
                .filter(x => x.visible && (x.txt.includes('下一页') || x.txt === '>' || x.txt === '›' || x.cls.includes('next') || x.aria.includes('下一页')))
                .map(x => ({idx:x.idx,txt:x.txt.slice(0,80),cls:x.cls.slice(0,120),aria:x.aria,role:x.role,disabled:x.disabled,small:x.small,rect:x.rect}))
                .slice(-40)
            };
          }
          ...
          return {ok:true, clicked:{idx:t.idx,txt:t.txt,cls:t.cls.slice(0,120),aria:t.aria,role:t.role,rect:t.rect}};
        }
        """
    )
...
def full_cycle_v6(page, state: Dict[str, Any]) -> int:
    processed = 0
    ...
    while page_no <= base.PAGE_MAX:
        ...
        next_res = click_product_all_next_page_v6(page, state)
