...
REPORT=Path('reports/hz20_mouse_click_recover_latest.json')
PAGE_SEQUENCE=os.environ.get('HZ20_PAGE_SEQUENCE','49')
LIMIT=int(os.environ.get('HZ20_LIMIT','15'))
WAIT=int(os.environ.get('HZ20_WAIT','10'))
FAIL_FUSE=int(os.environ.get('HZ20_FAIL_FUSE','6'))
ITEM_SLEEP_MIN=float(os.environ.get('HZ20_ITEM_SLEEP_MIN','1'))
ITEM_SLEEP_MAX=float(os.environ.get('HZ20_ITEM_SLEEP_MAX','3'))
RISK=['risk_handler','京东验证','快速验证','安全验证','验证码','滑块','购物无忧']

def now(): return datetime.now().strftime('%Y-%m-%d %H:%M:%S')
def log(event, **kw): print(json.dumps({'ts':now(),'worker':'hz20_mouse_click','event':event,**kw},ensure_ascii=False,sort_keys=True),flush=True)
def load_mod():
...
def dom_state(page):
    ...
    () => {
      ...
      const nodes=Array.from(document.querySelectorAll('.el-dialog,.ant-modal,[role=dialog],.modal,.el-message,.el-notification,.ant-message,.ant-notification'))
        .map((el,i)=>{const r=el.getBoundingClientRect();return {i,cls:String(el.className||'').slice(0,120),visible:r.width>0&&r.height>0,rect:{x:r.x,y:r.y,w:r.width,h:r.height},text:(el.innerText||el.textContent||'').slice(0,800)}})
        .filter(x=>x.visible||x.text);
      return {bodyHasShort:/https?:\/\/u\.jd\.com\//.test(txt),risk:['risk_handler','京东验证','快速验证','安全验证','验证码','滑块','购物无忧'].filter(x=>txt.includes(x)||location.href.includes(x)),nodes:nodes.slice(0,20)};
    }
    """)
...
def locate_button(page, sku:str, title:str)->Dict[str,Any]:
    return page.evaluate("""
    ({sku,title}) => {
      ...
      for (const b of buttons) {
        ...
        cands.push({score,reasons,rootText:raw.slice(0,500),links:links.slice(0,5),rootRect:{x:rr.x,y:rr.y,w:rr.width,h:rr.height},buttonRect:b.rect,buttonIndex:b.idx});
      }
      ...
      const t=cands[0];
      if (t.score<=0) return {ok:false,reason:'no_candidate_match',button_count:buttons.length,top:cands.slice(0,5)};
      return {ok:true,method:'card_scoped_locate',button_count:buttons.length,matched:t};
    }
    """, {'sku':sku,'title':title})

def physical_click(page, sku:str, title:str)->Dict[str,Any]:
    loc=locate_button(page,sku,title)
    if not loc.get('ok'): return loc
    br=(loc.get('matched') or {}).get('buttonRect') or {}
    # Move the matched button into the center of the browser viewport and then re-locate it.
    page.evaluate("""
    ({x,y}) => {
      const cx=x, cy=y;
    ...
    """, {'x':float(br.get('cx') or 0),'y':float(br.get('cy') or 0)})
    page.wait_for_timeout(500)
    loc2=locate_button(page,sku,title)
    if not loc2.get('ok'):
        loc['relocate']=loc2
        return loc
    br2=(loc2.get('matched') or {}).get('buttonRect') or {}
    x=float(br2.get('cx') or 0); y=float(br2.get('cy') or 0)
    page.mouse.move(x,y)
    page.wait_for_timeout(150)
    page.mouse.down()
    page.wait_for_timeout(120)
    page.mouse.up()
    loc2['mouse_click']={'x':x,'y':y}
    loc2['method']='card_scoped_physical_mouse_click'
    return loc2
...
def collect_one(page,cand:Dict[str,Any],state:Dict[str,Any],page_no:int,order:int)->Dict[str,Any]:
    ...
    try:
        ...
        click=physical_click(page,sku,title)
        if not click.get('ok'):
            return {'ok':False,'sku':sku,'reason':'physical_click_failed','click':click}
        result={}; last_dom={}
        ...
        row={'status':'ok','ts':now(),'worker_name':'hz20_mouse_click','source_menu':'商品推广/全部商品','menu_mode':'hz20_mouse_click_recovery','promotion_mode':'hz20_card_scoped_mouse_onekey','run_id':core.RUN_ID,'page_no':page_no,'page_order':order,'sku':sku,'sku_source':'hz20_card_candidate','title':title,'item_url':cand.get('itemUrl') or f'https://item.jd.com/{sku}.html','image_url':base.normalize_img(cand.get('imageUrl')),'price':cand.get('price'),'commission_rate':cand.get('rate'),'estimated_income':cand.get('income'),'short_url':result.get('short_url'),'long_url':result.get('long_url'),'qr_url':result.get('qr_url'),'jd_command':result.get('jd_command'),'link_created_at':created,'link_expire_at':expire,'link_expire_days':60,'refresh_due_at':refresh,'refresh_after_days':40,'refresh_before_expiry_days':20,'refresh_round':state.get('refresh_round',0),'click_result':click}
        core.append_jsonl(core.OUT,row)
        if sku and sku not in state['known_skus']: state['known_skus'].append(sku)
        state['fail_streak']=0; core.save_state(state); core.write_report(state,{'last_page_no':page_no,'last_hz20_sku':sku,'mode':'hz20_mouse_click_recovery'})
        log('ITEM_OK_HZ20',page_no=page_no,sku=sku,short_url=row.get('short_url'),known_sku_count=len(state.get('known_skus') or []))
...
def main():
    ...
    with sync_playwright() as p:
        ...
        if risk(page): rep.update(ok=False,reason='risk_initial',risk=risk(page))
            ...
            for page_no in rep['pages']:
                ...
                log('PAGE_CANDIDATES_HZ20',page_no=page_no,total=len(fresh),sample=[{'sku':x.get('sku'),'title':(x.get('title') or '')[:60]} for x in fresh[:8]])
                consecutive=0; page_ok=page_fail=0
                for order,c in enumerate(fresh[:LIMIT]):
                    log('ITEM_START_HZ20',page_no=page_no,order=order,sku=c.get('sku'),title=(c.get('title') or '')[:80])
                    r=collect_one(page,c,state,page_no,order); rep['results'].append({'page_no':page_no,'order':order,**r})
                    log('ITEM_RESULT_HZ20',page_no=page_no,sku=r.get('sku'),ok=r.get('ok'),reason=r.get('reason'),short_url=r.get('short_url'))
                    state=core.load_state(); state.setdefault('known_skus',[])
                    if r.get('ok'):
                        ...
                        if consecutive>=FAIL_FUSE:
                            log('PAGE_FAIL_FUSE_HZ20',page_no=page_no,consecutive_fail=consecutive,page_ok=page_ok,page_fail=page_fail); break
    ...
    log('HZ20_DONE',ok=rep.get('ok'),total_ok=rep.get('total_ok'),total_fail=rep.get('total_fail'),known_sku_count=rep.get('known_sku_count'))
